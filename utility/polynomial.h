//
// Created by xsQian on 2023/5/11.
//
...
*/
void polynomial_from_roots(std::vector<uint64_t> &roots,
                           std::vector<uint64_t> &coeffs, uint64_t modulus) {
  coeffs.clear();
  ...
  for (size_t i = 0; i < roots.size(); i++) {
    ...
    for (size_t j = i + 1; j > 0; j--) {
      coeffs[j] =
          (coeffs[j - 1] + MUL_MOD(neg_root, coeffs[j], modulus)) % modulus;
    }
...
/*
evaluate_polynomial(coeffs, x, modulus) computes the value of the polynomial
with coefficients coeffs at x, modulo modulus.

time complexity: O(n), where n is the size of coeffs
*/
uint64_t evaluate_polynomial(std::vector<uint64_t> &coeffs, uint64_t x,
                             uint64_t modulus) {
  uint64_t result = 0;
  for (size_t i = 0; i < coeffs.size(); i++) {
    result =
        (result + MUL_MOD(coeffs[i], modexp(x, i, modulus), modulus)) % modulus;
  }
  return result;
}
