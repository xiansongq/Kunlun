...
namespace OPRFPSI {
    ...
    std::vector<block> Receive(NetIO &io, OTEOPRF::PP &pp, std::vector<block> &vec_Y) {
        ...
        std::vector<uint8_t> vec_indication_bit(oprf_value.size());
...
//#pragma omp parallel for num_threads(thread_count)
//                for(int j=0;j<oprf_value[i].size();j++) {
//                    if(filter.Contain(oprf_value[i][j])) vec_indication_bit[i]=1;
...
//            }
            ...
            std::cout << "Receiver excute cuckoo filter query takes time= "
                      << std::chrono::duration<double, std::milli>(running_time1).count() << " ms" << std::endl;

            PrintSplitLine('-');
            std::cout << "mpOPRF-based PSI: Receive <=== CuckooFilter(F_k(x_i)) <=== Sender ["
            ...
            std::cout << "mpOPRF-based PSI [cuckoo filter]: Receiver side takes time "
...
#pragma omp parallel for num_threads(thread_count)
    ...
    void Send(NetIO &io, OTEOPRF::PP &pp, std::vector<block> &vec_X) {
        ...
        if (choice == "bloom") {
            ...
            auto bloom_end_time = std::chrono::steady_clock::now();
            auto running_time1 = bloom_end_time - bloom_start_time;
            std::cout << "Sender execute cuckoo filter insert takes time= "
                      << std::chrono::duration<double, std::milli>(running_time1).count() << " ms" << std::endl;
            ...
            std::cout << "mpOPRF-based PSI: Sender ===> CuckooFilter(F_k(x_i)) ===> Receiver ["
                      << (double) (filter_size) / (1024 * 1024) << " MB]" << std::endl;
            delete[] buffer;
            PrintSplitLine('-');
            auto end_time = std::chrono::steady_clock::now();
            auto running_time = end_time - start_time;
            std::cout << "mpOPRF-based PSI [cuckoo filter]: Sender side takes time "
                      << std::chrono::duration<double, std::milli>(running_time).count() << " ms" << std::endl;
        }
        if (choice == "plain") {
